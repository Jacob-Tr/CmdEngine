...
bool initMainThreadQueue(void)
{
	...
	if(!(initThread(main_thread, main_attr, &(mainLoop), NULL, 0) && initThread(input_thread, input_attr, &(inputLoop), NULL, 0)))
	{
		#ifdef DEBUG
		    fprintf(stderr, "Error: Could not initialize main and/or input thread. (%p and %p respectively)\n", main_thread, input_thread);
		#endif
