...
static vector2 chat_size = empty_vector2;
...
void addChat(const char* str, const size_t length)
{
	...
	size_t moves = 1;
	double d_moves = (((double) length) / ((double) chat_size.x));
	moves += (length >= chat_size.x) ? ((size_t) ceil(d_moves)) : 0;
	
	if(((d_moves - ((double) moves)) < 0.5) && moves > 1) --moves;
	moveChatUp(moves);
...
#include "includes/Engine/Interface/Commands/Commands.h"
