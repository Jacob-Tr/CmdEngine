...
#define EMPTY_THREADQUEUE ((threadQueue) {.init = false, .started = false, .size = 0, .entries = 0, .threads = NULL})
typedef struct
{
	bool init, started;
	size_t size, entries;
...
void joinQueue(threadQueue* queue)
{
	...
	#ifdef _MSC_VER
	    WaitForMultipleObjects(queue->entries, handles, true, INFINITE);
	#else
...
void startThread(THREAD* thread)
{
	...
	if(!(ret = THREAD_CREATE(&thread->handle, &thread->id, thread->stack_size, &thread->attr, thread->func_ptr, thread->func_param_buf, 0)))
	{
	    #ifdef DEBUG
