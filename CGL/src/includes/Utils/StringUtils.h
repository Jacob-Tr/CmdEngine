...
void strForm(char* dest, const char* src, const char align, const uint_8 strings, const size_t dest_size, const size_t lengths[])
{
	...
	{
        /*
            Each element of 'str_arr' is initialized to it's respective length plus the number of strings there are
            to account for necessary whitespace on the smallest string centering
        */
		*(str_arr + i) = (char*) malloc((sizeof(char) * (lengths[i] + 1)) + strings);
        // Zero each element of 'str_arr'.
		memset(str_arr[i], '\0', (sizeof(char) * lengths[i]));
		
        // Populate each element of 'str_arr' by extracting between the respective bounds given in the function parameters.
		substring(str_arr[i], src, index, (index + lengths[i]), (sizeof(char) * lengths[i]));
		
        // Get the index of the next alignment character.
		index = indexOf(str_arr[i], align, lengths[i]);

        // Conditionally zero index as indexOf may will input a -1 on fail. 
        if(index < 0) index = 0;
        ...
        // If the current index is the largest index 
		if(index_arr[i] > largest && index_arr[i] > 0) largest = index_arr[i];
	}
	
	const size_t add_max = (sizeof(char) * largest);
	
	char* new_str = (char*) malloc(sizeof(char) * (length + (largest * strings))), *add_str = (char*) malloc(add_max);
